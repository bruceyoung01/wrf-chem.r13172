...
!----------------------------------------------------------
apm_target_loc=get_location(target_loc)
!
if(localization_type == 1) then ! Standard Gaspari Cohn localization
   if(obs_type /= MOPITT_CO_RETRIEVAL) then
...
! APM +++ Rejection fix start 
   ...
   if(obs_type == MOPITT_CO_RETRIEVAL) then
      apm_pi=4.*atan(1.)
! scaled height
      if(apm_target_loc(3) > 1.217) then
         comp_cov_factor=apm_fac*0.
! scaled height
      else if(0.929 < apm_target_loc(3) .and. apm_target_loc(3) <= 1.217) then
         comp_cov_factor=apm_fac*sin(apm_pi/2.*(1.217-apm_target_loc(3))/0.288)
! scaled height
      else
         comp_cov_factor=apm_fac*1.
      endif
   endif
!
! APM --- Rejection fix end
!
